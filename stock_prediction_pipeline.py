# ============================================================
# stock_prediction_pipeline.py — Fixed Version
# Fixes: (1) paths use os.path relative to __file__
#        (2) yfinance uses period='2y' for guaranteed warmup
#        (3) MultiIndex column flattening made robust
# ============================================================

import warnings
warnings.filterwarnings("ignore")

import os
import numpy as np
import pandas as pd
import yfinance as yf
import joblib
import plotly.graph_objects as go
import plotly.express as px
from plotly.subplots import make_subplots

from sklearn.ensemble import RandomForestRegressor
from sklearn.preprocessing import RobustScaler
from sklearn.pipeline import Pipeline
from sklearn.metrics import mean_absolute_error, mean_squared_error, r2_score
from sklearn.base import BaseEstimator, TransformerMixin

# ─────────────────────────────────────────────
# 1. IMPORTING DEPENDENCIES
# ─────────────────────────────────────────────

# All imports are at the top of the file (see above).
print("✅ All dependencies imported successfully.")

# ─────────────────────────────────────────────
# PATH SETUP  ← always relative to this script
# ─────────────────────────────────────────────
BASE_DIR   = os.path.dirname(os.path.abspath(__file__))
DATA_DIR   = os.path.join(BASE_DIR, "data")
MODELS_DIR = os.path.join(BASE_DIR, "models")
os.makedirs(DATA_DIR,   exist_ok=True)
os.makedirs(MODELS_DIR, exist_ok=True)

# ─────────────────────────────────────────────
# 2. DATA FETCHING & CLEANING
# ─────────────────────────────────────────────
TICKER = "AAPL"


def fetch_stock_data(ticker: str) -> pd.DataFrame:
    """
    Fetch 2 years of daily OHLCV data via yfinance (period-based, not
    date-based) so we always get enough rows for SMA-200 + all lags.
    """
    print(f"\n📡 Fetching 2y of data for {ticker} (period='2y')...")
    raw = yf.download(
        ticker,
        period="2y",
        interval="1d",
        progress=False,
        auto_adjust=True,
        timeout=15,
    )

    if raw.empty:
        raise ValueError(f"No data returned for '{ticker}'. Check ticker symbol.")

    # Flatten multi-level columns (yfinance >= 0.2.x sometimes wraps them)
    if isinstance(raw.columns, pd.MultiIndex):
        raw.columns = raw.columns.get_level_values(0)

    df = raw.copy()
    df.index = pd.to_datetime(df.index)
    df.sort_index(inplace=True)

    # Forward / backward fill isolated gaps (holidays, weekends)
    df.ffill(inplace=True)
    df.bfill(inplace=True)

    # Remove extreme outliers with a 1-99 percentile IQR fence on Close
    Q1, Q3 = df["Close"].quantile(0.01), df["Close"].quantile(0.99)
    IQR = Q3 - Q1
    before = len(df)
    df = df[(df["Close"] >= Q1 - 3*IQR) & (df["Close"] <= Q3 + 3*IQR)]
    removed = before - len(df)
    if removed:
        print(f"  ⚠️  Removed {removed} outlier rows.")

    print(f"  ✅ Shape: {df.shape}  |  "
          f"{df.index[0].date()} → {df.index[-1].date()}")
    return df


df_raw = fetch_stock_data(TICKER)

# ─────────────────────────────────────────────
# 3. DATA VISUALIZATION
# ─────────────────────────────────────────────

def plot_candlestick(df: pd.DataFrame, ticker: str):
    fig = make_subplots(
        rows=2, cols=1, shared_xaxes=True,
        vertical_spacing=.03, row_heights=[.75, .25],
        subplot_titles=(f"{ticker} — Candlestick", "Volume"),
    )
    fig.add_trace(go.Candlestick(
        x=df.index,
        open=df["Open"], high=df["High"],
        low=df["Low"],   close=df["Close"],
        name="OHLC",
        increasing_line_color="#26a69a",
        decreasing_line_color="#ef5350",
    ), row=1, col=1)

    colors = ["#26a69a" if c >= o else "#ef5350"
              for c, o in zip(df["Close"], df["Open"])]
    fig.add_trace(go.Bar(x=df.index, y=df["Volume"],
                         name="Volume", marker_color=colors, opacity=.6), row=2, col=1)

    fig.update_layout(title=f"{ticker} Price & Volume",
                      xaxis_rangeslider_visible=False,
                      template="plotly_dark", height=700, showlegend=False)
    out = os.path.join(DATA_DIR, "candlestick_chart.html")
    fig.write_html(out)
    print(f"  📊 Candlestick chart → {out}")


plot_candlestick(df_raw, TICKER)

# ─────────────────────────────────────────────
# 4. EXPLORATORY DATA ANALYSIS (EDA)
# ─────────────────────────────────────────────

def engineer_features(df: pd.DataFrame) -> pd.DataFrame:
    """Build all 33 technical features. Must match app.py exactly."""
    d = df.copy()

    for w in [5, 10, 20, 50, 200]:
        d[f"SMA_{w}"] = d["Close"].rolling(w).mean()
        d[f"EMA_{w}"] = d["Close"].ewm(span=w, adjust=False).mean()

    d["BB_mid"]   = d["Close"].rolling(20).mean()
    d["BB_std"]   = d["Close"].rolling(20).std()
    d["BB_upper"] = d["BB_mid"] + 2 * d["BB_std"]
    d["BB_lower"] = d["BB_mid"] - 2 * d["BB_std"]
    d["BB_width"] = (d["BB_upper"] - d["BB_lower"]) / (d["BB_mid"] + 1e-9)

    delta = d["Close"].diff()
    gain  = delta.clip(lower=0).rolling(14).mean()
    loss  = (-delta.clip(upper=0)).rolling(14).mean()
    d["RSI_14"] = 100 - (100 / (1 + gain / (loss + 1e-9)))

    d["Daily_Return"]  = d["Close"].pct_change()
    d["Volatility_5"]  = d["Daily_Return"].rolling(5).std()
    d["Volatility_20"] = d["Daily_Return"].rolling(20).std()
    d["Momentum_5"]    = d["Close"] - d["Close"].shift(5)
    d["Momentum_10"]   = d["Close"] - d["Close"].shift(10)

    ema12 = d["Close"].ewm(span=12, adjust=False).mean()
    ema26 = d["Close"].ewm(span=26, adjust=False).mean()
    d["MACD"]        = ema12 - ema26
    d["MACD_signal"] = d["MACD"].ewm(span=9, adjust=False).mean()
    d["MACD_hist"]   = d["MACD"] - d["MACD_signal"]

    for lag in [1, 2, 3, 5, 10]:
        d[f"Close_lag_{lag}"]  = d["Close"].shift(lag)
        d[f"Return_lag_{lag}"] = d["Daily_Return"].shift(lag)

    d["Volume_SMA_10"] = d["Volume"].rolling(10).mean()
    d["Volume_ratio"]  = d["Volume"] / (d["Volume_SMA_10"] + 1e-9)
    d["DayOfWeek"]     = d.index.dayofweek
    d["Month"]         = d.index.month
    d["Quarter"]       = d.index.quarter
    return d


df_features = engineer_features(df_raw)


def plot_eda(df: pd.DataFrame, ticker: str):
    # Moving averages + Bollinger bands
    fig = go.Figure()
    fig.add_trace(go.Scatter(x=df.index, y=df["Close"],   name="Close",   line=dict(color="#fff",    width=1)))
    fig.add_trace(go.Scatter(x=df.index, y=df["SMA_20"],  name="SMA 20",  line=dict(color="#f59e0b", width=1.5)))
    fig.add_trace(go.Scatter(x=df.index, y=df["SMA_50"],  name="SMA 50",  line=dict(color="#3b82f6", width=1.5)))
    fig.add_trace(go.Scatter(x=df.index, y=df["SMA_200"], name="SMA 200", line=dict(color="#a855f7", width=1.5)))
    fig.add_trace(go.Scatter(x=df.index, y=df["BB_upper"],name="BB Upper",line=dict(color="#10b981", dash="dash", width=1)))
    fig.add_trace(go.Scatter(x=df.index, y=df["BB_lower"],name="BB Lower",line=dict(color="#ef4444", dash="dash", width=1),
                             fill="tonexty", fillcolor="rgba(16,185,129,.05)"))
    fig.update_layout(title=f"{ticker} — MAs & Bollinger Bands",
                      template="plotly_dark", height=500)
    fig.write_html(os.path.join(DATA_DIR, "eda_moving_averages.html"))

    # RSI
    fig2 = go.Figure()
    fig2.add_trace(go.Scatter(x=df.index, y=df["RSI_14"], name="RSI 14",
                              line=dict(color="#f59e0b", width=1.5)))
    fig2.add_hline(y=70, line_dash="dash", line_color="red",   annotation_text="Overbought (70)")
    fig2.add_hline(y=30, line_dash="dash", line_color="green", annotation_text="Oversold (30)")
    fig2.update_layout(title=f"{ticker} — RSI (14-day)",
                       template="plotly_dark", height=350, yaxis=dict(range=[0,100]))
    fig2.write_html(os.path.join(DATA_DIR, "eda_rsi.html"))

    # Correlation heatmap
    corr_cols = ["Close","SMA_20","SMA_50","EMA_20","RSI_14",
                 "MACD","BB_width","Volatility_20","Volume_ratio","Daily_Return"]
    corr_df = df[corr_cols].dropna().corr()
    fig3 = px.imshow(corr_df, text_auto=".2f", color_continuous_scale="RdBu_r",
                     title=f"{ticker} — Feature Correlation Heatmap",
                     template="plotly_dark", height=600)
    fig3.write_html(os.path.join(DATA_DIR, "eda_correlation_heatmap.html"))

    # Volatility
    fig4 = go.Figure()
    fig4.add_trace(go.Scatter(x=df.index, y=df["Volatility_20"]*100,
                              name="20-day Volatility (%)", line=dict(color="#a855f7")))
    fig4.update_layout(title=f"{ticker} — Rolling 20-day Volatility",
                       template="plotly_dark", yaxis_title="Volatility (%)", height=350)
    fig4.write_html(os.path.join(DATA_DIR, "eda_volatility.html"))

    print(f"  📊 EDA charts saved → {DATA_DIR}/")


plot_eda(df_features, TICKER)

# ─────────────────────────────────────────────
# 5. DATA MODELING
# ─────────────────────────────────────────────
MODEL_PARAMS = dict(
    n_estimators=300, max_depth=10,
    min_samples_split=5, min_samples_leaf=2,
    max_features="sqrt", random_state=42, n_jobs=-1,
)
print(f"\n🌲 Model: Random Forest  |  params: {MODEL_PARAMS}")

# ─────────────────────────────────────────────
# 6. SPLITTING DATA INTO FEATURES AND TARGETS
# ─────────────────────────────────────────────
FEATURE_COLS = [
    "Open", "High", "Low", "Volume",
    "SMA_5", "SMA_10", "SMA_20", "SMA_50",
    "EMA_5", "EMA_10", "EMA_20",
    "BB_width", "BB_upper", "BB_lower",
    "RSI_14", "MACD", "MACD_signal", "MACD_hist",
    "Volatility_5", "Volatility_20",
    "Momentum_5", "Momentum_10",
    "Close_lag_1", "Close_lag_2", "Close_lag_3", "Close_lag_5",
    "Return_lag_1", "Return_lag_2", "Return_lag_3",
    "Volume_ratio", "DayOfWeek", "Month", "Quarter",
]
TARGET_COL = "Close"

df_model = df_features[FEATURE_COLS + [TARGET_COL]].dropna().copy()
X = df_model[FEATURE_COLS].values
y = df_model[TARGET_COL].values
print(f"\n📐 Feature matrix: {X.shape}  |  Target: {y.shape}")

# ─────────────────────────────────────────────
# 7. SPLITTING INTO TRAINING AND TESTING SETS
# ─────────────────────────────────────────────
TEST_SIZE  = 0.15
split_idx  = int(len(X) * (1 - TEST_SIZE))
X_train, X_test = X[:split_idx], X[split_idx:]
y_train, y_test = y[:split_idx], y[split_idx:]
train_dates = df_model.index[:split_idx]
test_dates  = df_model.index[split_idx:]
print(f"\n✂️  Train: {len(X_train)} rows  |  Test: {len(X_test)} rows")

# ─────────────────────────────────────────────
# 8. MODEL TRAINING
# ─────────────────────────────────────────────
print("\n🏋️  Training Random Forest...")
scaler          = RobustScaler()
X_train_scaled  = scaler.fit_transform(X_train)
X_test_scaled   = scaler.transform(X_test)

model = RandomForestRegressor(**MODEL_PARAMS)
model.fit(X_train_scaled, y_train)
print("  ✅ Training complete.")

# ─────────────────────────────────────────────
# 9. MODEL EVALUATION
# ─────────────────────────────────────────────
y_pred = model.predict(X_test_scaled)
mae    = mean_absolute_error(y_test, y_pred)
rmse   = np.sqrt(mean_squared_error(y_test, y_pred))
r2     = r2_score(y_test, y_pred)
mape   = np.mean(np.abs((y_test - y_pred) / (y_test + 1e-9))) * 100

print("\n" + "="*50)
print("📈 EVALUATION")
print(f"  MAE  : ${mae:,.4f}")
print(f"  RMSE : ${rmse:,.4f}")
print(f"  R²   : {r2:.6f}")
print(f"  MAPE : {mape:.4f}%")
print("="*50)

feature_importance = pd.DataFrame({
    "Feature": FEATURE_COLS,
    "Importance": model.feature_importances_,
}).sort_values("Importance", ascending=False).head(15)
print("\n🔍 Top 15 features:\n", feature_importance.to_string(index=False))

# ─────────────────────────────────────────────
# 10. COMMUNICATION AND VISUALIZATION
# ─────────────────────────────────────────────

def plot_actual_vs_predicted(test_dates, y_test, y_pred, ticker: str):
    residuals = y_test - y_pred
    fig = make_subplots(
        rows=2, cols=1, shared_xaxes=True, vertical_spacing=.05,
        row_heights=[.7, .3],
        subplot_titles=(
            f"{ticker} — Actual vs Predicted (Test Set)",
            "Residuals (Actual − Predicted)",
        ),
    )
    fig.add_trace(go.Scatter(x=test_dates, y=y_test,  name="Actual",    line=dict(color="#60a5fa", width=2)), row=1, col=1)
    fig.add_trace(go.Scatter(x=test_dates, y=y_pred,  name="Predicted", line=dict(color="#f59e0b", width=2, dash="dot")), row=1, col=1)
    bar_colors = ["#ef4444" if r < 0 else "#10b981" for r in residuals]
    fig.add_trace(go.Bar(x=test_dates, y=residuals, name="Residuals",
                         marker_color=bar_colors, opacity=.6), row=2, col=1)
    fig.add_hline(y=0, line_dash="dash", line_color="gray", row=2, col=1)
    fig.update_layout(template="plotly_dark", height=650, showlegend=True)
    fig.write_html(os.path.join(DATA_DIR, "actual_vs_predicted.html"))

    fig2 = px.bar(feature_importance, x="Importance", y="Feature", orientation="h",
                  title=f"{ticker} — Top 15 Feature Importances",
                  color="Importance", color_continuous_scale="Viridis",
                  template="plotly_dark", height=500)
    fig2.update_layout(yaxis=dict(autorange="reversed"))
    fig2.write_html(os.path.join(DATA_DIR, "feature_importance.html"))
    print(f"  📊 Charts saved → {DATA_DIR}/")


plot_actual_vs_predicted(test_dates, y_test, y_pred, TICKER)

# ─────────────────────────────────────────────
# 11. MODEL SAVING
# ─────────────────────────────────────────────
MODEL_PATH  = os.path.join(MODELS_DIR, "stock_rf_model.sav")
SCALER_PATH = os.path.join(MODELS_DIR, "scaler.sav")
joblib.dump(model,  MODEL_PATH)
joblib.dump(scaler, SCALER_PATH)
print(f"\n💾 Model  → {MODEL_PATH}")
print(f"💾 Scaler → {SCALER_PATH}")

# ─────────────────────────────────────────────
# 12. DATA AUTOMATED PIPELINE IMPLEMENTATION
# ─────────────────────────────────────────────
stock_pipeline = Pipeline(steps=[
    ("scaler",    RobustScaler()),
    ("regressor", RandomForestRegressor(**MODEL_PARAMS)),
])
stock_pipeline.fit(X_train_scaled, y_train)   # fit on already-scaled data
pipeline_r2 = r2_score(y_test, stock_pipeline.predict(X_test_scaled))
print(f"\n🔧 sklearn Pipeline R² (verification): {pipeline_r2:.6f}")

# ─────────────────────────────────────────────
# 13. PIPELINE SAVING
# ─────────────────────────────────────────────
PIPELINE_PATH = os.path.join(MODELS_DIR, "stock_pipeline_compressed.sav")
joblib.dump(stock_pipeline, PIPELINE_PATH, compress=3)

metadata = {
    "ticker":          TICKER,
    "feature_cols":    FEATURE_COLS,
    "target_col":      TARGET_COL,
    "train_end_date":  str(train_dates[-1].date()),
    "test_mae":        round(mae,  4),
    "test_rmse":       round(rmse, 4),
    "test_r2":         round(r2,   6),
    "test_mape":       round(mape, 4),
}
META_PATH = os.path.join(MODELS_DIR, "pipeline_metadata.sav")
joblib.dump(metadata, META_PATH)

print(f"\n🗜️  Pipeline → {PIPELINE_PATH}")
print(f"📋 Metadata  → {META_PATH}")
print("\n🎉 Pipeline complete!  Run: streamlit run app.py")
